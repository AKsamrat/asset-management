...
import MyAssetData from './MyAssetData';
...
const MyAsset = () => {
  ...
  //pagination search filter sort load-data
  const {
    data: allAssets = [],
    isLoading,
    refetch,
    ...
    queryFn: async () => {
      const { data } = await axiosCommon(
        `/requested-assets/${user?.email}?page=${currentPage}&size=${itemperPage}&search=${search}&filter=${filter}&sort=${sort}`
      );
      console.log(data);
      return data;
    },
  ...
  const { data: assetCount } = useQuery({
    queryFn: async () => {
      const { data } = await axiosCommon(
        `/requestAssetsCountEmp/${user?.email}`
      );
      console.log(data);
      // refetch();
      setCount(data.count);
  ...
  return (
    <div className="mx-w-5xl pt-24 mx-auto px-5 md:px-8 lg:px-20 ">
      ...
      <h1 className="text-center text-4xl font-bold">
        My <span className="text-[#FEBF32]">Requested </span>Asset
      </h1>
      ...
      <div className="flex justify-center items-center gap-3">
        ...
        <div>
          <select
            ...
            className="px-1 md:px-4 py-2 border text-sm rounded-lg border-[#FEBF32]"
          >
            <option value="">Filter By Req. Status</option>
      ...
      <div className="py-8">
        <div className="  px-4 sm:px-8 py-4 overflow-x-auto mx-auto max-w-6xl">
