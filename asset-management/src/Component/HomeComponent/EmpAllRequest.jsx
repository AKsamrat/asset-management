...
const EmpAllRequest = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const { data } = await axiosSecure(`/empAll-request/${user?.email}`);
      console.log(data);
      return data;
    },
    queryKey: ['allAssets', user?.email],
  });

  return (
    <div className="mx-w-4xl pt-24 mx-auto px-5 md:px-8 lg:px-20 ">
      <h1 className="text-center text-4xl font-bold">
        Your last Month all <span className="text-[#FEBF32]">Request</span>
      </h1>
      <p className="text-center text-lg py-3">
        Here Your All Request WHich are made last month
      </p>
